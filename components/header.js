...
export default function Header({ theme, contact, pinned, active }) {
  ...
  let [showLogoSmall, setShowLogoSmall] = useState(false)
  ...
  useEffect(() => {
    ...
    if (scroll) {
      scroll.on('scroll', ({ limit, scroll }) => {
        ...
        if (progress > 0.5) {
          setShowLogoSmall(true)
        } else {
          setShowLogoSmall(false)
        } 
      })
    }
  ...
  return (
    ...
    {popupContext[0]?.popupEnabled && (
        <>
        {alertContext && (
          <div className="w-full bg-blue-dark text-white text-center text-xs md:text-sm px-4 absolute top-0 left-0 right-0 z-[50]">
    ...
    <header data-scroll data-scroll-sticky data-scroll-position="top" data-scroll-target="#scroll-container" className={`fixed top-0 left-0 right-0 z-30 header ${themeColors} ${pinned ? 'absolute top-0 left-0 right-0 w-full z-20 bg-opacity-0' : 'bg-noise'}`}>
      ...
      <div className={`pt-3 pb-3 md:pt-4 md:pb-4 xl:pt-5 xl:pb-5 flex items-center transition-all ease-in-out duration-500 overflow-visible ${showLogo ? 'h-[65px] md:h-[80px] 2xl:h-[80px]' : ' h-[110px] md:h-[140px] 2xl:h-[140px]'} ${(!showLogoSmall && alertContext && popupContext[0]?.popupEnabled) && 'header--alert'}`}>
        <Container>
