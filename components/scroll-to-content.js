...
export default function ScrollToContent({ blue }) {
  ...
  return (
    <button onClick={goToContent} className="flex space-x-3 items-center ring-white group">
      <span className={`border ${blue ? 'border-blue' : 'border-white'} border-opacity-50 rounded-full relative overflow-hidden group-hover:border-opacity-100 ease-in-out transition-all duration-500`}>
